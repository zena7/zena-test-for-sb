<script>
    ...
    const selectedCountry = {
      ...
      function roundUp(numb, index){
      return  Math.round((numb * index)*100)/100
    }



    function showErrorInput(value){
    ...
    function changeHandle(event){
      ...
      if (isNaN(event.target.value)){
        event.target.name === 'inputConvertFrom' ? errorMessageFrom.style.display = "block" : errorMessageTo.style.display  = 'block'
      } else {
        if (event.target.name === 'inputConvertFrom') {
        errorMessageFrom.style.display = 'none';
        inputTo = roundUp(event.target.value, index);
      } else {
        errorMessageTo.style.display = 'none';
        inputFrom = roundUp(event.target.value, 1 / index );
      }
...
<main>
  ...
  <div class="converter">
    <div class="blockFrom">
      ...
      <input type="text" name='inputConvertFrom' class ='inputFrom' on:input={changeHandle} value={ !!inputFrom || inputFrom === 0 ? inputFrom : ''} >
    ...
    <div class="blockTo">
      ...
      <input type="text"  name='inputConvertTo' class ='inputTo' on:input={changeHandle} value={ !!inputTo || inputFrom === 0  ? inputTo : ''} >
      <span class='loader'>Loading...</span>
      <p class="inputNaN">Please enter a valid amount</p>
    </div>
  </div>
...
.converter {
  ...
  select {
    /* color: white; */
/* background-color: rgba(107,160,249, 0.8); */
width: 25%;
padding: 0 4%;
